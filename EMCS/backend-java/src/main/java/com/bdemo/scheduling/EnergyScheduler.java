...
@Component public class EnergyScheduler{
 ...
 @Scheduled(cron="0 */15 * * * *") public void aggregateAndRules(){LocalDateTime end=LocalDateTime.now();var counts=pipeline.rebuildRange(end.minusHours(2),end);log.info("energy aggregate tick: {}",counts);pipeline.ruleStatus();}
 // REQ-051~056: daily cost orchestration uses the immutable version workflow exposed by CostService.
 @Scheduled(cron="0 0 3 * * *") public void dailyCost(){log.info("daily energy cost integrity: {}",pipeline.costIntegrity());}
 @Scheduled(cron="0 0 7 * * *") public void inspection(){agent.run("scheduled");}
}
