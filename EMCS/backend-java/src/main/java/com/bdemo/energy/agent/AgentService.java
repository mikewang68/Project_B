...
@Service public class AgentService{
 private final JdbcTemplate jdbc;private final ObjectMapper json;private final LocalDateTime now;public AgentService(JdbcTemplate j,ObjectMapper o,@Value("${b-demo.demo-now}")LocalDateTime n){jdbc=j;json=o;now=n;}
 public String answer(String message){Map<String,Object>s=jdbc.queryForMap("SELECT COUNT(*) alerts,(SELECT COUNT(*) FROM e_suggestion WHERE status NOT LIKE '%closed') suggestions FROM e_alert_event WHERE status NOT IN ('closed','false_closed')");return "基于当前演示数据：未关闭告警 "+s.get("alerts")+" 条，待闭环建议 "+s.get("suggestions")+" 条。你问的是："+message;}
 ...
 public long run(String trigger){Map<String,Object>s=jdbc.queryForMap("SELECT COUNT(*) openAlerts,(SELECT COUNT(*) FROM e_suggestion WHERE status NOT LIKE '%closed') openSuggestions FROM e_alert_event WHERE status NOT IN ('closed','false_closed')");String summary="巡检完成：发现未闭环告警 "+s.get("openAlerts")+" 条，待办建议 "+s.get("openSuggestions")+" 条。";KeyHolder k=new GeneratedKeyHolder();jdbc.update(c->{PreparedStatement p=c.prepareStatement("INSERT INTO ai_inspection_report(report_date,trigger_type,status,summary,findings_json,stats_json,model_name,elapsed_ms,created_at) VALUES(?,?,'completed',?,?,?,?,?,?)",Statement.RETURN_GENERATED_KEYS);p.setString(1,now.toLocalDate().toString());p.setString(2,trigger);p.setString(3,summary);p.setString(4,"[]");p.setString(5,toJson(s));p.setString(6,"deterministic-spring");p.setInt(7,1);p.setTimestamp(8,Timestamp.valueOf(now));return p;},k);return k.getKey().longValue();}
