...
class AppSettings(BaseSettings):
    ...
    # 演示日锚定（docs/mock-contracts.md 全局约定 2026-07-13）：
    # 空字符串表示未配置，回退 max(e_raw_reading.sample_time) 所在时刻，再兜底到 wall-clock。
    # 所有"当日/今日/now"口径（总览 KPI 窗口、趋势 nowIndex、告警"今日"计数、#14 聚合任务）统一走此锚。
    demo_now: str = ''
...
class LogSettings(BaseSettings):
    ...
    log_service_name: str = 'b-demo-backend'
