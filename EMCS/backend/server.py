...
from module_task.energy_scheduler import setup_energy_scheduler
...
async def _start_background_tasks(app: FastAPI) -> None:
    ...
    # 能源域定时任务：仅 leader worker 注册（scheduler 已 start）
    # REQ-020 重算触发·REQ-024~028 聚合·REQ-039~044 规则·REQ-060 总览刷新
    if SchedulerUtil._is_leader:
        setup_energy_scheduler(scheduler)
