-- B-Demo 骨架系统表初始化（源自 RuoYi-Vue3-FastAPI sql/ruoyi-fastapi.sql，MIT）
-- 已剥离：系统工具（表单构建/代码生成/系统接口）、AI 管理菜单及 ai_provider_type 字典
-- 业务表（能源域）由 datagen 后续迁移脚本单独管理
...
-- 初始化-部门表数据
-- ----------------------------
-- B 项目能源管控组织树：装卸一区/二区 dept 与 datagen 中的 e_area 通过 dept_id 关联（103/104）
insert into sys_dept values(100,  0,   '0',          'B 项目集团',      0, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(101,  100, '0,100',      '生产运营中心',    1, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(102,  100, '0,100',      '综合管理中心',    2, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(103,  101, '0,100,101',  'A 钢材装卸区',    1, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(104,  101, '0,100,101',  'B 粉煤灰筒仓区',  2, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(105,  101, '0,100,101',  '能源管理部',      3, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(106,  101, '0,100,101',  '调度中心',        4, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(107,  102, '0,100,102',  '运维部',          1, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(108,  102, '0,100,102',  '财务部',          2, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(109,  102, '0,100,102',  '系统运维组',      3, '', '138****0000', 'demo@example.com', '0', '0', 'admin', sysdate(), '', null);
...
-- 初始化-用户信息表数据
-- ----------------------------
-- K.6 admin｜dept=109 系统运维组｜口令 admin123（骨架默认，datagen 五角色沿用）
insert into sys_user values(1,  109, 'admin', '系统管理员', '00', 'admin@demo.example.com', '138****0001', '2', '', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', sysdate(), sysdate(), 'admin', sysdate(), '', null, 'PRD K.6 admin｜系统运维与审计');
...
-- 初始化-岗位信息表数据
-- ----------------------------
-- B 项目岗位（映射 PRD 附录 K.6 五角色的业务岗）
insert into sys_post values(1, 'energy_specialist', '能源管理专员', 1, '0', 'admin', sysdate(), '', null, 'K.6 能源管理员岗位');
insert into sys_post values(2, 'ops_engineer',      '运维工程师',   2, '0', 'admin', sysdate(), '', null, 'K.6 运维岗位');
insert into sys_post values(3, 'dispatcher',        '调度员',       3, '0', 'admin', sysdate(), '', null, 'K.6 调度岗位');
insert into sys_post values(4, 'finance_staff',     '财务专员',     4, '0', 'admin', sysdate(), '', null, 'K.6 财务岗位');
...
-- menu_id 111 数据监控（druid）已剔除：FastAPI 后端无 druid 端点，属 Java 骨架残留死链
...
-- 初始化-用户和角色关联表数据
-- ----------------------------
-- admin → 超级管理员角色；测试用户 niangao 已剔除
...
-- 初始化-用户与岗位关联表数据
-- ----------------------------
-- 骨架不预置岗位映射；K.6 演示账号的岗位关联由 datagen 按需追加
...
-- 初始化-公告信息表数据
-- ----------------------------
-- B-Demo 公告栏（占位内容，可在演示时人工替换）；上游 vfadmin/RuoYi 通知已剔除
insert into sys_notice values('1', 'B-Demo 上线公告：能源管控演示环境已启用', '2', '当前为构造数据演示环境，如需清空重建请重跑 datagen/generate_demo_data.py。', '0', 'admin', sysdate(), '', null, '演示公告');
insert into sys_notice values('2', '演示提醒：登录后请以角色矩阵为准查看数据范围',       '1', 'demo 五角色（energy_mgr / ops_user / dispatch_user / finance_user / admin）默认口令 admin123。', '0', 'admin', sysdate(), '', null, '演示提醒');
