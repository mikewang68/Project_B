...
import { createBusinessError } from '@/utils/requestError'
...
service.interceptors.response.use(async res => {
    ...
    if (code === 401) {
      ...
      return Promise.reject(createBusinessError(
        '无效的会话，或者会话已过期，请重新登录。', code, res.status
      ))
    } else if (code === 500) {
      ElMessage({ message: msg, type: 'error' })
      return Promise.reject(createBusinessError(msg, code, res.status))
    } else if (code === 601) {
  ...
  async error => {
    ...
    if (responseMsg) {
      ...
      return Promise.reject(createBusinessError(responseMsg, responseCode, responseStatus))
    }
    let { message } = error;
