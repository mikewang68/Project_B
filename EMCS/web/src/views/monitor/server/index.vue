<template>
  <div class="app-container">
    <el-row :gutter="10">
            ...
            <table cellspacing="0" style="width: 100%;">
              ...
              <tbody>
                <tr>
                  ...
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="server.jvmMemory">{{ server.jvmMemory.total }}</div></td>
                </tr>
                <tr>
                  <td class="el-table__cell is-leaf"><div class="cell">已用内存</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="server.mem">{{ server.mem.used}}</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="server.jvmMemory">{{ server.jvmMemory.used}}</div></td>
                </tr>
                <tr>
                  <td class="el-table__cell is-leaf"><div class="cell">剩余内存</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="server.mem">{{ server.mem.free }}</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="server.jvmMemory">{{ server.jvmMemory.free }}</div></td>
                </tr>
                <tr>
                  <td class="el-table__cell is-leaf"><div class="cell">使用率</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="server.mem" :class="{'text-danger': server.mem.usage > 80}">{{ server.mem.usage }}%</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="server.jvmMemory" :class="{'text-danger': server.jvmMemory.usage > 80}">{{ server.jvmMemory.usage }}%</div></td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <el-col :span="24" class="card-box">
        <el-card>
          <template #header><CoffeeCup style="width: 1em; height: 1em; vertical-align: middle;" /> <span style="vertical-align: middle;">Java 虚拟机信息</span></template>
          <div class="el-table el-table--enable-row-hover el-table--medium">
            <table cellspacing="0" style="width: 100%;table-layout:fixed;">
              <tbody>
                <tr>
                  <td class="el-table__cell is-leaf"><div class="cell">JVM 名称</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="server.jvm">{{ server.jvm.name }}</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell">Java 版本</div></td>
