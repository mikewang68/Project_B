...
const navMenu: NavItem[] = [
  { id: 'sys-dict', label: '数据字典', icon: 'Collection', path: '/maintain/dict', perm: 'sys:dict:type:view' },
  { id: 'sys-logs', label: '操作日志', icon: 'Document', path: '/maintain/logs', perm: 'sys:log:list:view' },
  { id: 'sys-config', label: '系统配置', icon: 'Setting', path: '/maintain/config', perm: 'sys:config:list:view' },
]
  ...
  <div class="admin-layout" :data-layout="prefs.layout" :class="{ collapsed: isSideCollapsed }">
    <aside v-show="prefs.layout !== 'top'" class="sidebar">
      <div class="sidebar-logo">
        ...
        <span v-if="!isSideCollapsed" class="logo-text">系统设置与维护</span>
      </div>
    ...
    <div class="main-area">
      <header class="topbar">
        <div v-if="prefs.layout === 'top'" class="topbar-brand">
          <div class="logo-icon logo-icon-sm">SYS</div>
          <span class="brand-name">系统设置与维护</span>
          <el-menu
            :default-active="activeMenu"
            mode="horizontal"
            class="top-menu"
        ...
        <div v-else class="topbar-left">
          <span class="page-title">{{ route.meta.title || '系统设置与维护' }}</span>
        </div>
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item disabled>
                  <span style="font-size:12px">账号：{{ auth.currentUser?.username }}</span>
                </el-dropdown-item>
