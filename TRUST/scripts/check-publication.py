...
def main():
    ...
    parser.add_argument('--trust-only', action='store_true', help='Scan only TRUST and the root .gitignore; leave other modules outside this audit.')
    ...
    scope = ['--', 'TRUST', '.gitignore'] if args.trust_only else []
    names = subprocess.check_output(['git', 'ls-files', '-z', '--cached', '--others', '--exclude-standard', *scope], cwd=ROOT).decode().split('\0')
    findings = []
    count = 0
    skipped = 0
    ...
    scope_label = 'TRUST and root .gitignore' if args.trust_only else 'whole repository'
    print(f'Scope: {scope_label}. Checked {count} upload candidate paths; {len(findings)} findings; {skipped} binary/large files skipped. Values omitted. Git history not covered.')
